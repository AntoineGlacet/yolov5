...
def plot_counting(
    W_line1, W_line2, totalUp, im, color=(0, 255, 255), line_thickness=2
):
    # Plots one bounding box on image 'im' using OpenCV
    assert (
        im.data.contiguous
    ...
    H = im.shape[1]
    cv2.line(im, (W_line1, 0), (W_line1, H), color, line_thickness)
    cv2.line(im, (W_line2, 0), (W_line2, H), color, line_thickness)

    text = "Pax= {}".format(totalUp)
    cv2.putText(
        im,
        text,
        (10, 50),
